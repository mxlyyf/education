...
import com.mxl.education.etl.dao.{AdsDaoByApi, AdsDaoBySql}
...
object AdsApp {
	def main(args: Array[String]): Unit = {
		...
		AdsDaoByApi.queryRegisterAppregurlnum(spark,"20190722")

		/*//需求4.统计通过各注册跳转地址(appregurl)进行注册的用户数
		AdsDaoBySql.queryRegisterAppregurlnum(spark, "20190722").show()
		//需求5.统计各所属网站（sitename）的用户数
		AdsDaoBySql.queryRegisterSitenamenum(spark, "20190722").show()
		//需求6.统计各所属平台的（regsourcename）用户数
		AdsDaoBySql.queryRegisterResourcenamenum(spark, "20190722").show()
		//需求7.统计通过各广告跳转（adname）的用户数
		AdsDaoBySql.queryRegisterAdnamenum(spark, "20190722").show()
		//需求8.统计各用户级别（memberlevel）的用户数
		AdsDaoBySql.queryRegisterMemberlevelnum(spark, "20190722").show()
		//需求9.统计各分区网站、用户级别下(website、memberlevel)的top3用户
		AdsDaoBySql.queryRegisterTop3member(spark,"20190722").show()*/
